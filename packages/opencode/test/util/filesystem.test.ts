...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "opensploit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "opensploit.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opensploit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opensploit.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["opensploit.json", "opensploit.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "opensploit.json"),
        path.join(tmp.path, "opensploit.jsonc"),
        path.join(project, "opensploit.json"),
        path.join(project, "opensploit.jsonc"),
      ])
    })
  })
