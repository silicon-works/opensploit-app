...
describe("websearch provider", () => {
  ...
  test("is enabled for all providers (OpenSploit override of upstream provider gating)", () => {
    expect(webSearchEnabled(ProviderID.opencode, { exa: false, parallel: false })).toBe(true)
    // OpenSploit enables websearch/codesearch for every provider (registry.ts webSearchEnabled → true),
    // so a non-opencode provider with no flags set is still enabled.
