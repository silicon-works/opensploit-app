...
async function writeManagedSettings(settings: object, filename = "opensploit.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "opensploit.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "opensploit.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "opensploit.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opensploit directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opensploit")
...
test("loads agents from .opensploit/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opensploit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .opensploit/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opensploit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .opensploit/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opensploit")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .opensploit config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".opensploit")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .opensploit config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "opensploit.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opensploit config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opensploit.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".opensploit")
...
test("local .opensploit config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opensploit directory config enables it
      const opencodeDir = path.join(dir, ".opensploit")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opensploit.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "opensploit.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.opensploit/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.opensploit/plugin/demo.ts", "file:///project/.opensploit/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.opensploit/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .opensploit/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .opensploit directory with a command
          const opencodeDir = path.join(dir, ".opensploit", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .opensploit should NOT be in directories list
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      ...
      PayloadIdentifier: "ai.opensploit.managed.test",
      PayloadType: "ai.opensploit.managed",
