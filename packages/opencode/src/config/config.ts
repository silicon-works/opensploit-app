...
function globalConfigFile() {
  const candidates = ["opensploit.jsonc", "opensploit.json", "opencode.jsonc", "opencode.json", "config.json"].map(
    (file) =>
  ...
  // No config exists yet. Default the create-path to the upstream-compatible
  // filename; the loader reads both brands, so an opensploit.jsonc a user
  // creates later still wins (it sorts ahead in the candidates above).
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opensploit.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opensploit.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const name of ["opencode", "opensploit"]) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opensploit") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["opencode.json", "opencode.jsonc", "opensploit.json", "opensploit.jsonc"]) {
