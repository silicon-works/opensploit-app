...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".opensploit"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".opensploit", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".opensploit", "tui.json"),
  }
}
