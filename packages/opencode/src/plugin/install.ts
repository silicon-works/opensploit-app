...
export type PatchDeps = {
  ...
  files: (dir: string, name: "opensploit" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".opensploit")
}

function patchName(kind: Kind): "opensploit" | "tui" {
  if (kind === "server") return "opensploit"
  return "tui"
}
